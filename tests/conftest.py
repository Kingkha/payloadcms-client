...
try:
    ...
    class _RequestsStub(types.ModuleType):
        class Session:  # type: ignore[override]
            def request(self, *args: Any, **kwargs: Any) -> Any:  # pragma: no cover - defensive
                raise NotImplementedError("requests.Session.request stubbed in tests")

    requests = _RequestsStub("requests")
    sys.modules["requests"] = requests
else:  # pragma: no cover
    requests  # noqa: F401 - expose real requests if available
