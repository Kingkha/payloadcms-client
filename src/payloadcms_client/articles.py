...
def _upload_document(
    ...
    *,
    ...
    media_root: str | Path | None,
    media_defaults: Mapping[str, Any] | None,
    media_filename_field: str,
    media_depth: int | None,
...
def upload_article_with_featured_image(
    client: "PayloadRESTClient",
    collection: str,
    file_path: str,
    featured_image: str | Path | None = None,
    *,
    builder: ArticlePayloadBuilder | None = None,
    depth: int | None = None,
    featured_image_field: str = "featuredImage",
    ...
    """Upload a single article and ensure the featured image is processed.

    When ``featured_image`` is provided the value is merged into the parsed
    front matter before uploading, which is useful for tests that prefer to
    keep fixtures image-agnostic. If the article already defines the featured
    image path in its front matter, simply omit ``featured_image`` and the
    helper will delegate to :func:`upload_article_from_file`.
    """
    ...
    metadata[featured_image_field] = str(featured_image)
    document_with_featured = ArticleDocument(
        metadata=metadata,
        body=document.body,
        raw=document.raw,
    )
    ...
    if effective_media_root is None:
        effective_media_root = Path(featured_image).resolve().parent

    return _upload_document(
        client,
        collection=collection,
        builder=builder,
        document=document_with_featured,
        article_path=article_path,
        depth=depth,
        featured_image_field=featured_image_field,
        media_collection=media_collection,
